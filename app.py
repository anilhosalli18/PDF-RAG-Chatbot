...
# Map pymupdf to fitz in sys.modules to suppress fitz deprecation warning
try:
    import pymupdf
...
try:
    ...
    t_logging.disable_default_handler()
except Exception:
    pass
...
# Suppress non-critical deprecation warnings and library advisory logs
...
def get_secret(key: str, default: str = None) -> str:
    ...
    try:
        if hasattr(st, "secrets") and st.secrets is not None:
            s_val = st.secrets.get(key, default)
            if s_val:
...
GROQ_MODELS = [
    "groq/compound",             # High capability Groq Compound model
    "groq/compound-mini",        # Fast & efficient Groq Compound mini
    "openai/gpt-oss-120b",       # High performance 120B OSS model
    "openai/gpt-oss-20b",        # Lightweight 20B OSS model
...
<div style='text-align: center; padding: 2rem 0 1rem 0;'>
    <h1 style='font-size: 4rem; margin-bottom: 0;'>&#128218; RAG Chatbot</h1>
...
with st.sidebar:
    ...
    # Initialize session state for API keys from environment/secrets if not set
    if "sidebar_groq_key" not in st.session_state or not st.session_state["sidebar_groq_key"]:
        st.session_state["sidebar_groq_key"] = get_secret("GROQ_API_KEY") or ""
    if "sidebar_openai_key" not in st.session_state or not st.session_state["sidebar_openai_key"]:
        st.session_state["sidebar_openai_key"] = get_secret("OPENAI_API_KEY") or ""

    if st.session_state["sidebar_groq_key"]:
        os.environ["GROQ_API_KEY"] = st.session_state["sidebar_groq_key"].strip()
    if st.session_state["sidebar_openai_key"]:
        os.environ["OPENAI_API_KEY"] = st.session_state["sidebar_openai_key"].strip()

    has_groq = bool(os.environ.get("GROQ_API_KEY"))
    has_openai = bool(os.environ.get("OPENAI_API_KEY"))

    # Interactive API Key Inputs in Sidebar
    with st.expander("&#128477; API Keys Configuration", expanded=not (has_groq or has_openai)):
        st.text_input("Groq API Key", key="sidebar_groq_key", type="password", help="Groq API key (starts with gsk_)")
        st.text_input("OpenAI API Key", key="sidebar_openai_key", type="password", help="OpenAI API key (starts with sk-)")
...
""")
                ...
                if st.session_state["messages"] and st.session_state["messages"][-1][0] == "user":
                    with st.spinner("&#129504; Thinking..."):
                        ...
                        groq_models = [(name, "groq") for name in GROQ_MODELS] if (GROQ_AVAILABLE and groq_key) else []
                        openai_models = [("gpt-3.5-turbo", "openai")] if (OPENAI_AVAILABLE and openai_key) else []
                        
                        if use_groq:
                            models_to_try = groq_models + openai_models
                        else:
                            models_to_try = openai_models + groq_models
                        
                        if not models_to_try:
                            st.error("&#10060; No LLM configured. Please set API keys in the sidebar.")
                            st.stop()
...
<div style='text-align: center; color: #666; padding: 20px;'>
    <p>ANIL H</p>
    <p>PDF RAG Chatbot • Built with LangChain, FAISS & Streamlit</p>
